/**
 * Deploy JLPT listening content and quizzes to Supabase
 * 1. Creates jlpt_listening_scripts table if not exists (migration)
 * 2. Inserts listening content (25 scripts)
 * 3. Inserts listening quizzes (5 quizzes, 50 questions)
 */
...
async function applyMigration() {
  console.log('1. Checking/creating jlpt_listening_scripts table...');

  const checkResp = await fetch(
    `${SUPABASE_URL}/rest/v1/jlpt_listening_scripts?select=id&limit=0`,
    { headers }
  );
  ...
  const migrationSql = fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'migrations', '00033_jlpt_listening_scripts.sql'),
    'utf8'
  );
  ...
  if (!resp.ok) {
    ...
    console.error('   File: supabase/migrations/00033_jlpt_listening_scripts.sql');
    return false;
  }
...
// ─── Step 2: Parse and insert listening content ───

function parseListeningContent() {
  const content = fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'seed_parts', 'jlpt_listening_content.sql'),
    'utf8'
  );

  const items = [];
  const insertRegex = /INSERT INTO jlpt_listening_scripts \([^)]+\)\s*VALUES \(\s*([\s\S]*?)\);/g;
  let match;

  while ((match = insertRegex.exec(content)) !== null) {
    const valuesStr = match[1];
    items.push(parseListeningValues(valuesStr));
  }

  return items;
}

function parseListeningValues(valuesStr) {
  const tokens = [];
  let current = '';
  let inString = false;
  let inArray = false;
  ...
  const script = unquote(tokens[1]);
  const script_type = unquote(tokens[2]);
  const jlpt_level = unquote(tokens[3]);
  const speakers = parseArray(tokens[4]);
  const situation = parseNull(tokens[5]);
  ...
  const translation_ko = parseNull(tokens[7]);
  const sort_order = parseInt(tokens[8]);
...
async function insertListeningContent(items) {
  console.log(`\n2. Inserting listening content (${items.length} scripts)...`);

  await fetch(`${SUPABASE_URL}/rest/v1/jlpt_listening_scripts?id=not.is.null`, {
    method: 'DELETE',
    headers,
  });
  console.log('   Cleared existing listening data');

  const batchSize = 5;
  for (let i = 0; i < items.length; i += batchSize) {
    const batch = items.slice(i, i + batchSize);
    const resp = await fetch(`${SUPABASE_URL}/rest/v1/jlpt_listening_scripts`, {
      method: 'POST',
  ...
  console.log(`   Done: ${items.length} listening scripts inserted`);
  return true;
}

// ─── Step 3: Insert listening quizzes ───
...
async function deleteExistingQuizData() {
  console.log('\n3. Deleting existing listening quiz data...');
  ...
  console.log('   Cleared existing listening quiz data');
}

async function insertListeningQuizzes() {
  console.log('\n4. Inserting listening quiz definitions...');
  const quizzes = [
    { id: QUIZ_IDS[0], title: 'N5 聴解クイズ', quiz_type: 'jlpt_listening', passing_score: 70, time_limit_minutes: 15, is_assessment: false },
    { id: QUIZ_IDS[1], title: 'N4 聴解クイズ', quiz_type: 'jlpt_listening', passing_score: 70, time_limit_minutes: 20, is_assessment: false },
    { id: QUIZ_IDS[2], title: 'N3 聴解クイズ', quiz_type: 'jlpt_listening', passing_score: 70, time_limit_minutes: 25, is_assessment: false },
    { id: QUIZ_IDS[3], title: 'N2 聴解クイズ', quiz_type: 'jlpt_listening', passing_score: 70, time_limit_minutes: 30, is_assessment: false },
    { id: QUIZ_IDS[4], title: 'N1 聴解クイズ', quiz_type: 'jlpt_listening', passing_score: 70, time_limit_minutes: 35, is_assessment: false },
  ];
  ...
  console.log('   OK - 5 listening quizzes inserted');
  return true;
}

function parseQuizQuestions() {
  const content = fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'seed_parts', 'jlpt_listening_quizzes.sql'),
    'utf8'
  );
...
async function main() {
  console.log('=== J-Bridge Listening Content Deployment ===\n');
  ...
  const listeningItems = parseListeningContent();
  console.log(`   Parsed ${listeningItems.length} listening scripts from seed file`);
  const contentOk = await insertListeningContent(listeningItems);
  if (!contentOk) {
    console.error('\nFailed to insert listening content.');
    process.exit(1);
  }

  await deleteExistingQuizData();

  const quizzesOk = await insertListeningQuizzes();
  if (!quizzesOk) {
    console.error('\nFailed to insert listening quizzes.');
    process.exit(1);
  }
  ...
  const listeningResp = await fetch(
    `${SUPABASE_URL}/rest/v1/jlpt_listening_scripts?select=jlpt_level`,
    { headers: { ...headers, 'Range': '0-999' } }
  );
  if (listeningResp.ok) {
    const scripts = await listeningResp.json();
    const byLevel = {};
    scripts.forEach(s => { byLevel[s.jlpt_level] = (byLevel[s.jlpt_level] || 0) + 1; });
    console.log(`  Listening scripts: ${scripts.length} total`);
