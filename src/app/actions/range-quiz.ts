...
export async function generateSharedVocabQuiz({
  category,
  rangeStart,
  rangeEnd,
  questionCount = 10,
  ...
  let query = supabase
    ...
    .select('id, term_ja, term_reading, term_ko')
    .eq('status', 'approved')
    .not('term_ko', 'is', null)
  ...
  const { data: poolData } = await poolQuery
  const allAnswers = [...new Set(poolData?.map(p => p.term_ko).filter(Boolean) ?? [])]
  ...
  const questions: QuizQuestion[] = selected.map(item => {
    ...
    return {
      id: item.id,
      question: item.term_ja,
      reading: item.term_reading ?? undefined,
      options,
    }
  })
