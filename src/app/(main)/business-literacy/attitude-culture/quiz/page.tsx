...
export const metadata = {
  title: '態度・企業文化 理解度テスト | J-Bridge',
  description: '態度・企業文化マニュアルの理解度テスト',
}

export default async function AttitudeCultureQuizListPage() {
  ...
  const { data: quizzes } = await supabase
    ...
    .or('title.ilike.%服務態度%,title.ilike.%ビジネスマナー%')
  ...
  return (
    <div>
      <div className="mb-6">
        <Link
          href="/business-literacy/attitude-culture"
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {allQuizzes.map((quiz) => {
            ...
            return (
              <Link
                key={quiz.id}
                href={`/business-literacy/attitude-culture/quiz/${quiz.id}`}
