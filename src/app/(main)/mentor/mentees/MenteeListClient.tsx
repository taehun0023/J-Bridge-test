...
import { ClipboardList, ChevronDown, CheckCircle2, Clock, AlertTriangle, Eye } from 'lucide-react'
...
/** JLPT系（模試/旧総合試験）は N レベルで等級表示、それ以外は S/A/B/C/D */
const JLPT_EXAM_CATEGORIES = new Set(['jlpt-mock', 'seikatsu'])
...
type CompExamResult = MenteeOverview['compExamResults'][number]

/** 総合試験 카테고리 표시 순서 (레이더 축 순서와 동일) */
const EXAM_CATEGORIES_NON_JP = ['jlpt-mock', 'business-jp', 'cs', 'dev', 'business-lit']
const EXAM_CATEGORIES_JP = ['cs', 'dev', 'business-lit']

/** 미수험 플레이스홀더 — 기록이 없어도 카테고리 행을 항상 표시 */
function notTakenRow(category: string): CompExamResult {
  return { id: `__not-taken-${category}`, category, contentLevel: null, score: null, passed: false, status: 'not_taken', completedAt: null }
}
...
function MenteeAccordion({ mentee }: { mentee: MenteeOverview }) {
  ...
  // 모든 総合試験 카테고리를 항상 노출 — 응시 기록이 있으면 그 결과, 없으면 未受験
  const compResults = mentee.compExamResults
  const byCategory = new Map(compResults.map(r => [r.category, r]))
  const expectedCats = mentee.is_japanese ? EXAM_CATEGORIES_JP : EXAM_CATEGORIES_NON_JP
  const displayCompResults: CompExamResult[] = [
    ...expectedCats.map(c => byCategory.get(c) ?? notTakenRow(c)),
    ...compResults.filter(r => !expectedCats.includes(r.category)),
  ]
            ...
            <div className="flex flex-col justify-center space-y-3 text-sm">
              ...
              <Link href="/admin/tasks" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group">
                ...
                <span className="font-medium group-hover:underline">
                  ...
                  {mentee.assignmentsOverdue > 0 && (
                    <> / 遅延 <span className="text-red-600">{mentee.assignmentsOverdue}</span></>
                  )}
                </span>
              </Link>
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {displayCompResults.map(r => {
                      const isJlptExam = JLPT_EXAM_CATEGORIES.has(r.category)
                      const notTaken = r.status === 'not_taken'
                      const gradeLabel = r.score !== null
                        ? (isJlptExam ? (r.contentLevel ?? getJlptLevel(r.score)) : getGrade(r.score))
                        : null
                      const gradeColor = gradeLabel !== null
                        ? (isJlptExam ? getJlptLevelColor(gradeLabel as JlptLevel) : getGradeColor(gradeLabel as SkillGrade))
                        : ''
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2 pr-4">
                            {notTaken ? (
                              <span className="text-xs font-medium text-zinc-400 dark:text-zinc-500">未受験</span>
                            ) : (
...
export default function MenteeListClient({ mentees }: { mentees: MenteeOverview[] }) {
  if (mentees.length === 0) {
    return (
      <Card>
