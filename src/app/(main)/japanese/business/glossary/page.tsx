...
const QUIZ_TYPE = 'it_terminology'
...
export default async function GlossaryPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { count: totalInCategory } = await supabase
    ...
    .in('category', ALL_GLOSSARY_CATEGORIES)

  const totalItems = totalInCategory ?? 0
  const masteredInCategory = masteredIds.length > 0
    ? (await supabase.from('it_glossary').select('id').in('category', ALL_GLOSSARY_CATEGORIES).in('id', masteredIds)).data?.length ?? 0
    : 0
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">ビジネス用語</h1>
