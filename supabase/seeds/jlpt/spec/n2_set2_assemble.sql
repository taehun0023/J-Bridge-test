-- N2 공식구성 Set2(107문) 조립: 컨테이너 3개 → jlpt_mock_set_questions (section + daimon + sort_order)
-- set_id = aa000002-0000-0000-0000-000000000022 (N2 set_no=2)
BEGIN;

DELETE FROM jlpt_mock_set_questions WHERE set_id = 'aa000002-0000-0000-0000-000000000022';
...
SET time_limit_minutes = 155,
    ...
    is_published = true
WHERE id = 'aa000002-0000-0000-0000-000000000022';

INSERT INTO jlpt_mock_set_questions (set_id, question_id, section, daimon, sort_order)
SELECT 'aa000002-0000-0000-0000-000000000022', qq.id,
  CASE qq.quiz_id
    WHEN 'ad000002-0000-0000-0000-000000000221' THEN 'gengo_chishiki'
    WHEN 'ad000002-0000-0000-0000-000000000222' THEN 'dokkai'
    WHEN 'ad000002-0000-0000-0000-000000000223' THEN 'choukai'
  END AS section,
...
WHERE qq.quiz_id IN (
  'ad000002-0000-0000-0000-000000000221',
...
WHERE set_id='aa000002-0000-0000-0000-000000000022' GROUP BY section ORDER BY section;
