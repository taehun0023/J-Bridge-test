-- N2 공식구성 Set5(107문) 조립: 컨테이너 3개 → jlpt_mock_set_questions (section + daimon + sort_order)
-- set_id = aa000002-0000-0000-0000-000000000025 (N2 set_no=5)
BEGIN;

DELETE FROM jlpt_mock_set_questions WHERE set_id = 'aa000002-0000-0000-0000-000000000025';
...
SET time_limit_minutes = 155,
    ...
    is_published = true
WHERE id = 'aa000002-0000-0000-0000-000000000025';

INSERT INTO jlpt_mock_set_questions (set_id, question_id, section, daimon, sort_order)
SELECT 'aa000002-0000-0000-0000-000000000025', qq.id,
  CASE qq.quiz_id
    WHEN 'ad000002-0000-0000-0000-000000000251' THEN 'gengo_chishiki'
    WHEN 'ad000002-0000-0000-0000-000000000252' THEN 'dokkai'
    WHEN 'ad000002-0000-0000-0000-000000000253' THEN 'choukai'
  END AS section,
...
WHERE qq.quiz_id IN (
  'ad000002-0000-0000-0000-000000000251',
...
WHERE set_id='aa000002-0000-0000-0000-000000000025' GROUP BY section ORDER BY section;
