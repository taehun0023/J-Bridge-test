-- N2 공식구성 Set1(107문) 조립: 컨테이너 3개 → jlpt_mock_set_questions (section + daimon + sort_order)
-- set_id = aa000002-0000-0000-0000-000000000021 (N2 set_no=1)
BEGIN;

DELETE FROM jlpt_mock_set_questions WHERE set_id = 'aa000002-0000-0000-0000-000000000021';
...
SET time_limit_minutes = 155,
    ...
    is_published = true
WHERE id = 'aa000002-0000-0000-0000-000000000021';

INSERT INTO jlpt_mock_set_questions (set_id, question_id, section, daimon, sort_order)
SELECT 'aa000002-0000-0000-0000-000000000021', qq.id,
  CASE qq.quiz_id
    WHEN 'ad000002-0000-0000-0000-000000000211' THEN 'gengo_chishiki'
    WHEN 'ad000002-0000-0000-0000-000000000212' THEN 'dokkai'
    WHEN 'ad000002-0000-0000-0000-000000000213' THEN 'choukai'
  END AS section,
...
WHERE qq.quiz_id IN (
  'ad000002-0000-0000-0000-000000000211',
...
WHERE set_id='aa000002-0000-0000-0000-000000000021' GROUP BY section ORDER BY section;
